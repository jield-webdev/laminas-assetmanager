...
class AliasPathStackResolverTest extends TestCase
{
    ...
    public function testAddAlias()
    {
        ...
        $this->assertEquals(
            ['alias' => 'path' . DIRECTORY_SEPARATOR],
            $property->getValue($resolver)
        );
    }
    ...
    public function testGetAndSetMimeResolver()
    {
        ...
        $resolver = new AliasPathStackResolver(['my/alias/' => __DIR__]);
    ...
    {
        $resolver     = new AliasPathStackResolver(['AliasPathStackResolverTest/' => __DIR__]);
        $mimeResolver = new MimeResolver();
        $resolver->setMimeResolver($mimeResolver);
        $fileAsset           = new Asset\FileAsset(__FILE__);
        $fileAsset->mimetype = $mimeResolver->getMimeType(__FILE__);
    ...
    {
        $alias    = 'my/alias/';
        $resolver = new AliasPathStackResolver([$alias => __DIR__]);
    ...
    {
        $alias    = 'my/alias/';
        $resolver = new AliasPathStackResolver([$alias => realpath(__DIR__ . '/../')]);
